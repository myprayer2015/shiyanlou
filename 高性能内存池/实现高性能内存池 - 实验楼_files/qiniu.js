...
function QiniuJsSDK() {
    ...
    this.getFileExtension = function(filename) {
        ...
        var ext;
        if (tempArr.length === 1 || (tempArr[0] === "" && tempArr.length ===
                2)) {
            ext = "";
        } else {
    ...
    this.utf8_encode = function(argString) {
        ...
        for (var n = 0; n < stringl; n++) {
            ...
            if (c1 < 128) {
                ...
                enc = String.fromCharCode(
                    (c1 >> 12) | 224, ((c1 >> 6) & 63) | 128, (c1 & 63) |
                    128
                );
            } else { // surrogate pairs
                if (c1 & 0xFC00 ^ 0xD800 > 0) {
                    throw new RangeError('Unmatched trail surrogate at ' +
                        n);
                }
    ...
    this.uploader = function(op) {
        ...
        var reset_chunk_size = function() {
            ...
            var isSpecialSafari = (mOxie.Env.browser === "Safari" &&
                mOxie.Env.version <= 5 && mOxie.Env.os ===
                "Windows" && mOxie.Env.osVersion === "7") || (mOxie
                .Env.browser === "Safari" && mOxie.Env.os === "iOS" &&
        ...
        var getFileKey = function(up, file, func) {
            var key = '',
                unique_names = false;
            if (!op.save_key) {
                unique_names = up.getOption && up.getOption(
                    'unique_names');
                unique_names = unique_names || (up.settings && up.settings
                    .unique_names);
                if (unique_names) {
        ...
        uploader.bind('FilesAdded', function(up, files) {
            var auto_start = up.getOption && up.getOption(
                'auto_start');
        ...
        uploader.bind('BeforeUpload', function(up, file) {
            ...
            var directUpload = function(up, file, func) {
                ...
                if (x_vars !== undefined && typeof x_vars ===
                    'object') {
                    for (var x_key in x_vars) {
                        if (x_vars.hasOwnProperty(x_key)) {
                            if (typeof x_vars[x_key] ===
                                'function') {
                                multipart_params_obj['x:' +
                                    x_key] = x_vars[x_key](
                                    up, file);
                            } else if (typeof x_vars[x_key] !==
                                'object') {
                                multipart_params_obj['x:' +
                                    x_key] = x_vars[x_key];
                            }
            ...
            var chunk_size = up.getOption && up.getOption(
                'chunk_size');
            ...
            if (uploader.runtime === 'html5' && chunk_size) {
                if (file.size < chunk_size) {
                    ...
                    up.setOption({
                        'url': 'https://upload.qiniup.com/mkblk/' +
                            blockSize,
                        'multipart': false,
                ...
                if (file) {
                    switch (err.code) {
                        ...
                        case plupload.FILE_SIZE_ERROR:
                            var max_file_size = up.getOption &&
                                up.getOption(
                                    'max_file_size');
            ...
            return function(up, file, info) {
                ...
                var last_step = function(up, file, info) {
                    if (op.downtoken_url) {
                        ...
                        ajax_downtoken.open('POST', op.downtoken_url,
                            true);
                        ajax_downtoken.setRequestHeader(
                            'Content-type',
                            'application/x-www-form-urlencoded'
                        );
                        ajax_downtoken.onreadystatechange =
                            function() {
                                if (ajax_downtoken.readyState ===
                                    4) {
                                    if (ajax_downtoken.status ===
                                        200) {
                                        var
                                            res_downtoken;
                                        try {
                                        ...
                                        uploader.trigger(
                                            'Error', {
                                                status: ajax_downtoken
                                                    .status,
                                                response: ajax_downtoken
                                                    .responseText,
                                                file: file,
                                                code: plupload
                                                    .HTTP_ERROR
                        ...
                        ajax_downtoken.send('key=' +
                            that.parseJSON(info).key +
                            '&domain=' + op.domain);
                    } else if (_FileUploaded_Handler) {
                        _FileUploaded_Handler(up, file,
                            info);
                    }
                };
                ...
                if (ctx) {
                    ...
                    if (!op.save_key) {
                        ...
                        key = key ? '/key/' + that.URLSafeBase64Encode(
                            key) : '';
                    }
                    ...
                    var url =
                        'https://upload.qiniup.com/mkfile/' +
                        file.size + key + x_vars_url;
                    var ajax = that.createAjax();
